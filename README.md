...
**Note this is detailed documentation, a very short version generated by Sphinx is at** [Installation guide.pdf](https://github.com/Mak2006/python-rest-app-cont/blob/master/Installation%20guide.pdf)
...
1. Create Github repo and load the project in PyCharm. 
...
1. We use the generic directory structure, our app is `mathapp` and a module, tests are separated, we create the `__init__.py` empty file, so that we can treat `mathapp` as a module
```
mathapp/
...
tests/            
    - functional  
    - unit
docs/  # this is the documentation generated by Sphinx
    
```


### Test run the application. 
We run `server.py` using the IDE. The application would be available at the stock URL 
